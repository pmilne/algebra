...
a ^ n =
  if n < 0
    then error "Collins: Negative exponent"
    else powerAssociative (*) one a n
...
subresultant rtn u v =
  (if deg u > deg v
    ...
    one
    one -- avoid passing the rtn function in the recursion
        -- degree u >= degree v
  where
    rec u v g h =
      trace
        ("subresultant " ++ "\t\tu: " ++ show u ++ "\t\tv: " ++ show v ++ "\t\tv: " ++ show g ++ "\t\tv: " ++ show h) $
      if v == zero
        then rtn u zero
        else let delta = deg u - deg v
             in let lcv = lc v
               -- The first clause is only required in the first iteration where delta may be zero, requiring an explicit check.
                in let nh =
                         if delta == 0 && h == one
                           then lcv
                           else (lcv ^ delta) /! (h ^ (delta - 1))
                   in if deg v == 0
                        then rtn one nh
...
gcd1 u v =
  let g = subresultant _gcd u v
  in sign g * g

instance (Show a, Eq a, Ring a, Euclidean a) => Euclidean (Polynomial a) where
  divide = divide1
  gcd u v =
    let d = gcd (content u) (content v)
    in Const d * pp (gcd1 (pp u) (pp v))
