...
run = do
          ...
          testDerivative (Lambda y (y + pc 1)) (Lambda y (pc 1))
          testDerivative (Lambda y (pc 3 * y ^ pc 2)) (Lambda y (pc 6 * y))
          testDerivative (Lambda y (y ^ y)) (Lambda y (y ^ y * (pc 1 + ln y)))
          testDerivative (Lambda y (ln (ln y))) (Lambda y (pc 1 / (y * ln y)))

          testDerivative (Lambda x (sin x)) (Lambda x (cos x))
          testDerivative (Lambda x (x + sin x)) (Lambda x (one + cos x))
          testDerivative (Lambda x (sin (sin x))) (Lambda x (cos x * cos (sin x)))
          testDerivative (Lambda x (tan (tan x))) (Lambda x (one/((cos x ^ two)*(cos (tan x) ^ two))))
          testDerivative (Lambda x (asin x)) (Lambda x (one / sqrt (one + neg (x ^ two))))
          testDerivative (Lambda x (tan x)) (Lambda x (one / cos x^two))
          testDerivative (Lambda x (sin x / cos x)) (Lambda x (((sin x ^ two) /(cos x ^ two)) + one))
